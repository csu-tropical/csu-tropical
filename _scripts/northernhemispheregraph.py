...
#If it is January 1st, reset observed northernhemisphere ACE'
if (currentmonth==1):
	if(currentday==1):
...
currentace = float(globalarray[0][9])+float(globalarray[1][9])+float(globalarray[2][9])+float(globalarray[3][9])
...
basinacetodatetable = open('../_data/tc_stats/northernhemisphereacetodate.txt', 'w')
count = 0
...
#Append observed data for as many days as there have been this year
	if (count < basindays):
		basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][43], "-", currentyear, ",", basinarray[count][1], ",", seasonalcyclearray[count][44], "\n")))
		count = count + 1
#Append remainder of climatological curve
	else:
		basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][43], "-", currentyear, ",,",  seasonalcyclearray[count][44], "\n")))
		count = count + 1
