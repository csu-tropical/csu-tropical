...
if (currenthour>=18):
	basinacetodatetable = open('../_data/tc_stats/southernhemisphereacetodate.txt', 'a')
	...
	value = float(globalarray[4][9])+float(globalarray[5][9])
	value = round(value, 1)
	value = str(value)
...
#Append observed data for as many days as there have been this year
		if (count < basindays):
			basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], ",", basinarray[count][0], ",", seasonalcyclearray[count][48], "\n")))
			count = count + 1
#Append remainder of climatological curve
		else:
			basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], ",,",  seasonalcyclearray[count][48], "\n")))
			count = count + 1
