...
if (currentmonth==1):
	if(currentday==1):
		os.remove('../_data/tc_stats/atlanticacetodate.txt', 'w')
...
basinacetodatetable = open('../_data/tc_stats/atlanticacetodate.txt', 'r')
...
basinacetodatetable = open('../_data/tc_stats/atlanticacetodate.txt', 'w')
count = 0
...
basinacetodatevsclimotable = open('../_data/tc_stats/atlanticacetodatevsclimo.csv', 'w')
...
#Append observed data for as many days as there have been this year
	if (count < basindays):
		basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][43], "-", currentyear, ",", basinarray[count][1], ",", seasonalcyclearray[count][7], "\n")))
		count = count + 1
#Append remainder of climatological curve
	else:
		basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][43], "-", currentyear, ",,",  seasonalcyclearray[count][7], "\n")))
		count = count + 1
