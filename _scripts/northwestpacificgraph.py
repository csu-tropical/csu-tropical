...
if (currenthour>=18):
	basinacetodatetable = open('../_data/tc_stats/northwestpacificacetodate.txt', 'a')
...
#Append observed data for as many days as there have been this year
		if (count < basindays):
			basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][43], ",", basinarray[count][0], ",", seasonalcyclearray[count][21], "\n")))
			count = count + 1
#Append remainder of climatological curve
		else:
			basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][43], ",,",  seasonalcyclearray[count][21], "\n")))
			count = count + 1
