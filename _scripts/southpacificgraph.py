...
#If it is January 1st, reset observed southpacific ACE'
if (currentmonth==1):
	if(currentday==1):
		os.remove('../_data/tc_stats/southpacificacetodate.txt', 'w')
...
basinacetodatetable = open('../_data/tc_stats/southpacificacetodate.txt', 'r')
...
basinacetodatetable = open('../_data/tc_stats/southpacificacetodate.txt', 'w')
count = 0
...
basinacetodatevsclimotable = open('../_data/tc_stats/southpacificacetodatevsclimo.csv', 'w')
...
#Append observed data for as many days as there have been this year
	if (count < basindays):
            if (count < 184):
                basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], "-", lastyear, ",", basinarray[count][1], ",", seasonalcyclearray[count][47], "\n")))
            else:
                basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], "-", currentyear, ",", basinarray[count][1], ",", seasonalcyclearray[count][47], "\n")))
            count = count + 1
#Append remainder of climatological curve
	else:
            if (count < 184):
                basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], "-", lastyear, ",,",  seasonalcyclearray[count][47], "\n")))
            else:
                basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], "-", currentyear, ",,",  seasonalcyclearray[count][47], "\n")))
            count = count + 1
