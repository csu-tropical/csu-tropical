...
if (currenthour>=18):
	basinacetodatetable = open('../_data/tc_stats/southindianacetodate.txt', 'a')
...
#Append observed data for as many days as there have been this year
		if (count < basindays):
			basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], ",", basinarray[count][0], ",", seasonalcyclearray[count][46], "\n")))
			count = count + 1
#Append remainder of climatological curve
		else:
			basinacetodatevsclimotable.write(''.join((seasonalcyclearray[count][45], ",,",  seasonalcyclearray[count][46], "\n")))
			count = count + 1
